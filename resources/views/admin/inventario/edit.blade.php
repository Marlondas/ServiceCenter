...
<head>
    ...
    <title>Editar Producto - Service Center</title>
...
<body>
    ...
    <div class="page-loader">
        ...
        <div class="loader-subtitle">Cargando Edición de Producto...</div>
        ...
        <main class="main-content">
            ...
            <div class="status-card">
                <h2 class="status-card-title">
                    <i class="fas fa-info-circle"></i> Estado Actual del Producto
                </h2>
                <div class="status-details">
                    <div class="status-item">
                        <span class="status-label">Nombre</span>
                        <span class="status-value">{{ $producto->nombre }}</span>
                    </div>
                    
                    <div class="status-item">
                        <span class="status-label">Cantidad Actual</span>
                        <span class="status-value">{{ $producto->cantidad }}</span>
                    </div>
                    
                    <div class="status-item">
                        <span class="status-label">Stock Mínimo</span>
                        <span class="status-value">{{ $producto->stock_minimo }}</span>
                    </div>
            ...
            <div class="form-container">
                <div class="form-header">
                    <h2 class="form-title">
                        <i class="fas fa-edit"></i> Editar Información del Producto
                    </h2>
                    <p class="form-subtitle">Actualice los detalles del producto según sea necesario</p>
                </div>
                
                <form action="{{ route('admin.inventario.update', $producto->id_producto) }}" method="POST" id="product-form">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="cantidad" name="cantidad" value="{{ old('cantidad', $producto->cantidad) }}" min="0" class="form-control" required>
                        <span class="form-text">La cantidad actual disponible en inventario.</span>
                    </div>
                    
                    <div class="form-group">
                        <label for="stock_minimo" class="form-label">Stock Mínimo</label>
                        <input type="number" id="stock_minimo" name="stock_minimo" value="{{ old('stock_minimo', $producto->stock_minimo) }}" min="1" class="form-control" required>
            ...
            <div class="action-panel">
                <a href="{{ route('admin.inventario.movimientoForm', $producto->id_producto) }}" class="action-btn">
                    <i class="fas fa-exchange-alt"></i>
                    <div class="action-title">Registrar Movimiento</div>
                    <div class="action-text">Registre entradas o salidas de este producto</div>
                </a>
                
                <a href="{{ route('admin.inventario.verMovimientos', $producto->id_producto) }}" class="action-btn">
                    <i class="fas fa-history"></i>
                    <div class="action-title">Ver Historial</div>
                    <div class="action-text">Consulte el historial de movimientos de este producto</div>
                </a>
                
                <form action="{{ route('admin.inventario.destroy', $producto->id_producto) }}" method="POST" style="display: contents;" onsubmit="return confirm('¿Está seguro de eliminar este producto? Esta acción no se puede deshacer.')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="action-btn" style="border: none; cursor: pointer; width: 100%;">
                        <i class="fas fa-trash-alt"></i>
                        <div class="action-title">Eliminar Producto</div>
                        <div class="action-text">Elimine permanentemente este producto</div>
                    </button>
                </form>
            </div>
        </main>
