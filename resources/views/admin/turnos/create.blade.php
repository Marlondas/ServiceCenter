...
<head>
    ...
    <title>Crear Turno - Service Center</title>
...
<body>
                ...
                <form method="POST" action="{{ route('admin.turnos.store') }}">
                    ...
                    <div class="form-grid">
                            ...
                            <select id="id_vehiculo" name="id_vehiculo" class="form-select" required>
                                ...
                                @foreach($vehiculos as $vehiculo)
                                    <option value="{{ $vehiculo->id_vehiculo }}" {{ old('id_vehiculo') == $vehiculo->id_vehiculo ? 'selected' : '' }}
                                        ...
                                        {{ $vehiculo->placa }} ({{ ucfirst($vehiculo->tipo_vehiculo ?? 'carro') }} - Cliente: {{ $vehiculo->cliente->usuario->nombre ?? 'No disponible' }})
                                    </option>
                                @endforeach
                            </select>
                            @error('id_vehiculo')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="fecha" name="fecha" class="form-control datepicker" value="{{ old('fecha') }}" required>
                            @error('fecha')
                                <div class="form-error">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="hora" name="hora" class="form-control timepicker" value="{{ old('hora') }}" required>
                            @error('hora')
                                <div class="form-error">{{ $message }}</div>
                            @enderror
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Cuando cambie el vehículo seleccionado
            $('#id_vehiculo').change(function() {
                var vehiculoId = $(this).val();
                if (vehiculoId) {
                    // Realizar petición AJAX para obtener servicios para ese tipo de vehículo
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.length === 0) {
                                ...
                                $.each(data, function(key, value) {
                                    servicioSelect.append('<option value="' + value.id_servicio + '">' + 
            ...
            // Disparar el evento change al cargar la página si hay un vehículo seleccionado
            if ($('#id_vehiculo').val()) {
